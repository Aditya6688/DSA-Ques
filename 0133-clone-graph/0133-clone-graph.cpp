...
public:
    ...
    void dfs(Node*actual, Node*clone){
        for(auto neighbor: actual->neighbors){ //all neighbors of actual node
            if(nodelist[neighbor->val]==NULL){ //neighbor's clone not created
            ...
            else{//neighbor's clone already exists
